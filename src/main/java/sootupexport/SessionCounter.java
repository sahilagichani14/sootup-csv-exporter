...
public class SessionCounter {
  ...
  /** Keep the current count of temporary vars of a certain kind, identified by base name. */
  private final Map<String, Integer> _tempVarMap = new ConcurrentHashMap<>();
  /**
   * Keeps the unique index of an instruction in the method. This cannot be computed up
   * front,because temporary variables (and assignments to them from constants) will be inserted
   * while the IR is being processed.
   */
  ...
  public int getInstructionIndex(Object u) {
    ...
    if (result == null) throw new RuntimeException("No unit number available for '" + u + "'");
    return result;
  }
}
